...
class MinistryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = \App\Ministry::select(\DB::raw('*'));
        if ($keywords) {
    ...
    {
        return response()->json(['data' => \App\Ministry::findOrFail($id)]);
    }
     ...
     * }
     */
    public function update(MinistriesUpdateRequest $request, $id)
    {
        try {
            $input = $request->only(['name', 'descriptions']);
            $input['updated_by'] = auth()->user()->id;

            if ($result = \App\Ministry::find($id)->update($input)) {
    ...
    {
        return response()->json(['success' => \App\Ministry::findOrFail($id)->delete()]);
    }
}
