...
use App\Http\Resources\SundayService as SundayServiceResources;
...
class SundayServiceController extends Controller
{
    ...
    public function create(Request $request, $id, $year, $week) {
        try {
            ...
            $yearweek = $year.$week;
            $attendance = \App\Member::where('parent_id', $id)->withSundayService($yearweek)->get();
            return response()->json(['ok' => true, 'data' => SundayServiceResources::collection($attendance)], 201);    
    ...
    public function store(Request $request, $id, $year, $week) {
        try {
            ...
            $yearweek = $year.$week;
            $attendance = \App\SundayCelebration::whereRaw('YEARWEEK(`date_attended`, 1) = "'. $yearweek .'"')
                ->where('member_id', $input['member_id'])->first();

            if ($attendance) {
                $attendance->updated_by = Auth()->user()->id;
            } else {
                $attendance = new \App\SundayCelebration();
            ...
            $success = false;
            $data = '';
            $attendance->date_attended = isset($input['date_attended']) && $input['date_attended'] != 'null' ? date('Y-m-d H:i:s', strtotime($input['date_attended'])) : date('Y-m-d H:i:s');
            $week = date('YW', strtotime($attendance->date_attended));
            if ($attendance->save()) {
                $success = true;
                $a = \App\Member::withSundayService($week)->whereRaw('members.id = '. $input['member_id'])->first();

                $data = new SundayServiceResources($a);
            } else {
